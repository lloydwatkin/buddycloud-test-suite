 ...
 * $RCSfile$
 * $Revision: 11825 $
 * $Date: 2010-08-15 12:45:57 -0300 (dom, 15 ago 2010) $
 *
...
import org.buddycloud.channelserver.TestIQPacket;
...
public class PacketParserUtils {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                // this element name and namespace.
                    ...
                    if (provider != null) {
                    	...
                    	Element element = parseElement(parser);
                    	StringWriter out = new StringWriter();
						new XMLOutputter().output(element, out);
						out.flush();
						iqPacket = new TestIQPacket(out.toString());
                    }
                }
            }
            else if (eventType == XmlPullParser.END_TAG) {
	...
	private static Element parseElement(XmlPullParser parser)
			throws XmlPullParserException, IOException {
		Element el = new Element(parser.getName());
		el.setNamespace(Namespace.getNamespace(parser.getNamespace()));

		for (int i = 0; i < parser.getAttributeCount(); i++) {
			el.setAttribute(parser.getAttributeName(i), parser.getAttributeValue(i));
		}
		...
		while (true) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				el.addContent(parseElement(parser));
			} else if (eventType == XmlPullParser.TEXT){
				el.setText(parser.getText());
			} else if (eventType == XmlPullParser.END_TAG){
				return el;
			}
		}
